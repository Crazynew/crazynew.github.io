...
* 抽象父类可以只定义所需方法，把不能实现的部门抽象成抽象方法，留给其子类去实现；
...
* 修饰符可以使public或者省略，省略代表默认采用宝权限访问控制符；
...
* 在接口里定义长远变量时，不管是否使用public static final修饰符，接口里的成员变量总是使用这三个修饰符来修饰。
...
## 内部类
定义在其他类内部的类就被称为内部类（潜逃类），包含内部类的类也被称为外部类（宿主类）。Java从JDK1.1开始引入内部类，主要作用在于：
...
##### 局部内部类
定义在方法中的内部类即是局部内部类，其仅在方法里有效，不能使用访问控制符和static修饰符。如果需要用局部内不良定义变量、创建实例或派生子类，都只能在局部内部了所在方法内进行。正式因为局部内部类的作用域太小了，在实际开发中很少用到局部内部类。
...
* 枚举类可以实现一个或多个接口，使用enum定义的枚举类默认继承了java.lang.Enum类，而不是默认继承Object类，因为枚举类不能显式继承其他父类。其中java.lang.Enum类实现了java.lang.Serializable和java.lang.Comparable两个接口；
